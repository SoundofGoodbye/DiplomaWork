...
public class FunctionalityManager implements IsSerializable
{
	private Date startDate = new Date();

	private Date endDate = new Date();

	private ArrayList<Person> leaders = new ArrayList<Person>();

	private ArrayList<Person> reviewers = new ArrayList<Person>();

	private ArrayList<DiplomaWork> softwareWorks = new ArrayList<DiplomaWork>();
	...
	private ArrayList<Defence> netDefences = new ArrayList<Defence>();

	private ArrayList<Defence> hardDefences = new ArrayList<Defence>();

	private ArrayList<Defence> softDefences = new ArrayList<Defence>();

	private static FunctionalityManager FM;

	public FunctionalityManager()
	{
	}

	public static synchronized FunctionalityManager getInstance()
	{
		if (FM == null)
		{
			FM = new FunctionalityManager();
		}
		return FM;
	}
	...
	public void getDiploma(	String projectName,
							String diplomants,
							String diplomaLeader,
							String diplomaReviewer,
							String specialtie,
							String type)
	{
		...
		for (Person currentDiplomaLeader : leaders)
		{
			String currentDiplomaLeaderName = currentDiplomaLeader.getName();
			if (currentDiplomaLeaderName.equals(diplomaLeader))
			{
				leader = currentDiplomaLeader;
				break;
			}
		}

		for (Person currentReviewer : reviewers)
		{
			String currentReviewerName = currentReviewer.getName();
			if (currentReviewerName.equals(diplomaReviewer))
			{
		...
		if (specialtie.equalsIgnoreCase("Software")
				&& !softwareWorks.contains(new DiplomaWork(projectName, diplomants, leader, reviewer, type)))
		{
			softwareWorks.add(new DiplomaWork(projectName, diplomants, leader, reviewer, type));
		}
		else if (specialtie.equalsIgnoreCase("Hardware")
				&& !hardwareWorks.contains(new DiplomaWork(projectName, diplomants, leader, reviewer, "Hardware")))
		{
			hardwareWorks.add(new DiplomaWork(projectName, diplomants, leader, reviewer, "Hardware"));
		}
		else if (specialtie.equalsIgnoreCase("Communications")
				&& !netWorks.contains(new DiplomaWork(projectName, diplomants, leader, reviewer, "Communications")))
		{
			netWorks.add(new DiplomaWork(projectName, diplomants, leader, reviewer, "Communication"));
		}
	}
	...
	public Defence defaultGeneration(ArrayList<DiplomaWork> diplomaWorksList, HashSet<Date> takenDates)
	{
		Defence defence = new Defence();
		
		//There may be a maximum of nine defenses over one week
		if (diplomaWorksList.size() <= 9)
		{
			for (DiplomaWork currentDiplomaWork : diplomaWorksList)
			{
				defence.addDiploma(currentDiplomaWork);
			}
			diplomaWorksList.clear();
		}
		//If there are less then nine defenses over one week:
		else
		{
			for (int i = 0; i < 8; i++)
			{
				defence.addDiploma(diplomaWorksList.get(i));
			}
			diplomaWorksList.removeAll(defence.getDiplomaWorks());
		}

		generateJury(defence, diplomaWorksList);
	...
	private void setDefenceDate(Defence defence, HashSet<Date> takenDates)
	{
		...
		{
			if (defence.getJury().get(i).getAvailableDates().size() < busyMan.getAvailableDates().size())
			{
				busyMan = defence.getJury().get(i);
			}
		}
	...
	private void generateJury(Defence defence, ArrayList<DiplomaWork> diplomaWorksList)
	{
		...
		{
			int k = 0;
			while (jury.size() < 4 && k < diplomaWorksList.size())
			{
				addPersonToJury(jury, diplomaWorksList.get(k));
				k++;
			}
		}
	...
	public void organizeLastSoftwareWorks(ArrayList<Defence> defences, HashSet<Date> takenDates)
	{
